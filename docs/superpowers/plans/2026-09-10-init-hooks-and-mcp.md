...
- `export PATH="$HOME/.cargo/bin:$PATH"` before any cargo command. Corpus checkouts (the FastLift checkout, the StrongSpan checkout, the Teyji checkout, the autoQA checkout, the FastLift admin checkout) are never written into: `init` is exercised on a temporary copy of `fastlift-admin`, and the hook benchmark on `fasting-app` uses a fresh `LOCRIN_CACHE_DIR` and `--root`, which reads only.
