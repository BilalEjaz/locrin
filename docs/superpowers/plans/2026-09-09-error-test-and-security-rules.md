...
- **Task 8, the table ships 158 entries rather than 100.** The plan asked for at least a hundred and named about ninety; the rest are the providers the founder's own stack uses (Expo, RevenueCat, OneSignal, AppsFlyer, Branch, Statsig) and the near neighbours of entries already listed. Every one of them has a must-flag and a must-not-flag fixture line, and the fixture test fails if a new entry arrives without them.
